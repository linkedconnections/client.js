...
HttpFetcher.prototype.request = function (url, methodName) {
  ...
  function execute() {
    ...
    var headers = { 'Accept': 'application/ld+json;q=1.0',
        ...
        settings = { hostname: parsedUrl.host, path: parsedUrl.path, headers: headers, withCredentials: false, timeout: 5000, method: method},
  ...
  function onResponse(error, response, body) {
    ...
    if (typeof window !== "undefined") {
      responseUrl = self._active[requestId].request.xhr.responseURL;
    } else if (response.fetchedUrls.length > 1) {
